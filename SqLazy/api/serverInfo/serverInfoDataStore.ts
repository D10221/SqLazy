...
var servers:IDataStore<IServerInfo> = new Datastore({filename: x, autoload: true});
...
OnEmpty((store)=> {
    ...
    store.insert([{
        ...
        (err, newDocs) => {
            if (err) throw err;
            if (newDocs) console.info("Success inserting ");
        });
});

function FindAll(callback:(error:Error, data:IServerInfo[])=>void) {
    // Find all documents in the collection
    //Remember function signature , there is no d.td, db.find({}, function (err, docs) {});
    servers.find({}, callback);
}

function OnEmpty(onEmpty:(x:IDataStore<IServerInfo>) => void, onError?:(xerror:Error, xstore:IDataStore<IServerInfo>) =>void) {
    FindAll((error, result)=> {
        if (error) {
            if (onError) {
